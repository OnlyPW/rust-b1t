...
fn denomination_from_str(mut s: &str) -> Option<Denomination> {
    if s.eq_ignore_ascii_case("BTC") {
        return Some(Denomination::Pepecoin);
    }
...
impl Amount {
    ...
    pub fn from_btc(btc: f64) -> Result<Amount, ParseAmountError> {
        Amount::from_float_in(btc, Denomination::Pepecoin)
    }
    ...
    /// # Examples
    /// ```
    /// # use nintondo_pepecoin::{Amount, Denomination};
    /// let amount = Amount::from_sat(100_000);
    /// assert_eq!(amount.to_btc(), amount.to_float_in(Denomination::Pepecoin))
    /// ```
    pub fn to_btc(self) -> f64 {
        self.to_float_in(Denomination::Pepecoin)
    }
...
impl fmt::Display for Amount {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        self.fmt_value_in(f, Denomination::Pepecoin)?;
        write!(f, " {}", Denomination::Pepecoin)
    }
}
    ...
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        ...
        match &self.style {
            ...
            DisplayStyle::DynamicDenomination if self.sats_abs >= Amount::ONE_BTC.to_sat() => {
                fmt_satoshi_in(self.sats_abs, self.is_negative, f, Denomination::Pepecoin, true, format_options)
            },
            DisplayStyle::DynamicDenomination => {
...
impl SignedAmount {
    ...
    pub fn from_btc(btc: f64) -> Result<SignedAmount, ParseAmountError> {
        SignedAmount::from_float_in(btc, Denomination::Pepecoin)
    }
    ...
    ///
    /// Equivalent to `to_float_in(Denomination::Pepecoin)`.
    ///
    /// Please be aware of the risk of using floating-point numbers.
    pub fn to_btc(self) -> f64 {
        self.to_float_in(Denomination::Pepecoin)
...
pub mod serde {
    ...
    //!     #[serde(with = "nintondo_pepecoin::amount::serde::as_btc")]
    //!     pub amount: Amount,
    //! }
    //! ```
    ...
    impl SerdeAmount for Amount {
        ...
        fn ser_btc<S: Serializer>(self, s: S) -> Result<S::Ok, S::Error> {
            f64::serialize(&self.to_float_in(Denomination::Pepecoin), s)
        }
        fn des_btc<'d, D: Deserializer<'d>>(d: D) -> Result<Self, D::Error> {
            use serde::de::Error;
...
mod tests {
    ...
    fn floating_point() {
        ...
        assert_eq!(f(11.22, D::Pepecoin), Ok(sat(1122000000)));
        ...
        assert_eq!(f(0.0001234, D::Pepecoin), Ok(sat(12340)));
        assert_eq!(sf(-0.00012345, D::Pepecoin), Ok(ssat(-12345)));
        ...
        assert_eq!(sf(-100.0, D::MilliSatoshi), Err(ParseAmountError::TooPrecise));
        assert_eq!(f(42.123456781, D::Pepecoin), Err(ParseAmountError::TooPrecise));
        assert_eq!(sf(-184467440738.0, D::Pepecoin), Err(ParseAmountError::TooBig));
        ...
        assert_eq!(btc(2.5).to_float_in(D::Pepecoin), 2.5);
        ...
        assert_eq!(&btc(0.0012).to_float_in(D::Pepecoin).to_string(), "0.0012")
    }
    ...
    fn parsing() {
        ...
        // exactly 50 chars.
        assert_eq!(
            p("100000000000000.0000000000000000000000000000000000", Denomination::Pepecoin),
            Err(E::TooBig)
        );
        // more than 50 chars.
        assert_eq!(
            p("100000000000000.00000000000000000000000000000000000", Denomination::Pepecoin),
            Err(E::InputTooLarge)
        );
    }
    ...
    fn to_string() {
        ...
        assert_eq!(Amount::ONE_BTC.to_string_in(D::Pepecoin), "1");
        assert_eq!(format!("{:.8}", Amount::ONE_BTC.display_in(D::Pepecoin)), "1.00000000");
        assert_eq!(Amount::ONE_BTC.to_string_in(D::Satoshi), "100000000");
        assert_eq!(Amount::ONE_SAT.to_string_in(D::Pepecoin), "0.00000001");
        assert_eq!(SignedAmount::from_sat(-42).to_string_in(D::Pepecoin), "-0.00000042");

        assert_eq!(Amount::ONE_BTC.to_string_with_denomination(D::Pepecoin), "1 BTC");
        ...
        assert_eq!(Amount::ONE_SAT.to_string_with_denomination(D::Pepecoin), "0.00000001 BTC");
        assert_eq!(
            SignedAmount::from_sat(-42).to_string_with_denomination(D::Pepecoin),
            "-0.00000042 BTC"
        );
    }
    ...
    fn to_from_string_in() {
        ...
        assert_eq!("0.00253583", Amount::from_sat(253583).to_string_in(D::Pepecoin));
        assert_eq!("-5", SignedAmount::from_sat(-5).to_string_in(D::Satoshi));
        assert_eq!("0.1", Amount::from_sat(100_000_00).to_string_in(D::Pepecoin));
        ...
        assert_eq!(
            "0.10000000",
            format!("{:.8}", Amount::from_sat(100_000_00).display_in(D::Pepecoin))
        );
        ...
        assert_eq!(ua_str(&ua_sat(500).to_string_in(D::Pepecoin), D::Pepecoin), Ok(ua_sat(500)));
        assert_eq!(
    ...
    fn to_string_with_denomination_from_str_roundtrip() {
        ...
        assert_eq!(Amount::from_str(&denom(amt, D::Pepecoin)), Ok(amt));
